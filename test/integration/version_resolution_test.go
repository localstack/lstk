...
func TestVersionResolvedViaImageInspection(t *testing.T) {
	...
	must.NoError(t, err, "lstk start failed:\nstdout: %s\nstderr: %s", stdout, stderr)

	must.NotEmpty(t, *capturedVersion, "license request should carry a version resolved from image inspection")
	must.NotEq(t, "latest", *capturedVersion, "resolved version should not be the unresolved 'latest' tag")
	must.Contains(t, stdout, "Checking license")

	semverLike := strings.Contains(*capturedVersion, ".") || strings.Contains(*capturedVersion, "-")
	must.True(t, semverLike, "resolved version %q should look like a real version", *capturedVersion)
}
...
func TestCommandFailsNicelyWhenLicenseCheckFails(t *testing.T) {
	...
	must.Error(t, err, "expected lstk start to fail when license check fails")
...
`), 0644))
	...
	out := stdout + "\n" + stderr
	must.Eq(t, "dev", *capturedVersion, "the configured tag should be sent to the license API")
	must.NotContains(t, out, "license validation failed",
		"an unparseable tag must not be treated as a license rejection")
	must.Contains(t, out, `does not support tag "dev"`,
		"the skipped pre-flight should be surfaced as a warning")
	must.Error(t, err, "the start should proceed to the pull and fail on the unreachable registry")
	requireExitCode(t, 1, err)
	must.Contains(t, out, "Failed to pull")
}
...
`), 0644))
	...
	must.Contains(t, stdout, "License validation failed",
		"stderr: %s", stderr)
	must.Eq(t, "4.0.0", *capturedVersion, "pinned tag should be sent directly to the license API without image inspection")
}
