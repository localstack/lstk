...
// snapshot). The latter used to exit 0 because Cobra prints help and returns
// nil for an unrecognized subcommand of a non-runnable parent.
...
func TestBareParentCommandExitsZero(t *testing.T) {
	...
	for _, parent := range []string{"config", "setup", "volume", "snapshot"} {
		parent := parent
