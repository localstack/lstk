...
func TestStartTypeFlagFirstRunCreatesConfig(t *testing.T) {
	...
	must.Contains(t, stdout, "Snowflake emulator selected.")
	data, err := os.ReadFile(configPath)
	must.NoError(t, err)
	must.Contains(t, string(data), `type = "snowflake"`)
}
...
func TestBareRootTypeFlagCreatesConfig(t *testing.T) {
	...
	must.Contains(t, stdout, "Azure emulator selected.")
	data, err := os.ReadFile(configPath)
	must.NoError(t, err)
	must.Contains(t, string(data), `type = "azure"`)
}
...
func TestStartTypeFlagSwitchesInPlace(t *testing.T) {
	...
	must.Contains(t, stdout, "Switched configured emulator to Azure")
	data, err := os.ReadFile(configPath)
	must.NoError(t, err)
	must.Contains(t, string(data), `type = "azure"`)
	// The surgical rewrite preserves the inline comment and other fields.
	must.Contains(t, string(data), "# keep me")
	must.Contains(t, string(data), `port = "4566"`)
}
...
func TestStartTypeFlagNoOpWhenMatching(t *testing.T) {
	...
	must.NoError(t, os.MkdirAll(filepath.Dir(configPath), 0755))
	content := "[[containers]]\ntype = \"aws\"\ntag = \"latest\"\nport = \"4566\"\n"
	must.NoError(t, os.WriteFile(configPath, []byte(content), 0644))

	stdout, _, _ := runLstk(t, testContext(t), t.TempDir(), e, "start", "--type", "aws", "--non-interactive")

	must.NotContains(t, stdout, "Switched configured emulator")
	data, err := os.ReadFile(configPath)
	must.NoError(t, err)
	must.Eq(t, content, string(data))
}
...
func TestStartTypeFlagErrorsWhenImageSet(t *testing.T) {
	...
	must.Error(t, err)
	must.Contains(t, stdout, "Cannot switch emulator to Snowflake while a custom image is set")
...
func TestStartTypeErrorsOnMultipleBlocks(t *testing.T) {
	...
	must.Error(t, err)
	must.Contains(t, stdout, "Unsupported configuration")
...
func TestStartTypePositionalRejected(t *testing.T) {
	...
	must.Error(t, err)
	must.Contains(t, stderr, "select the emulator with --type")
	must.NoFileExists(t, configPath)
}
...
func TestStartTypeErrorsWhenNoContainersBlock(t *testing.T) {
	...
	must.Error(t, err)
	must.Contains(t, stderr, "[[containers]] block")
...
func TestStartTypeInvalidValue(t *testing.T) {
	...
	must.Error(t, err)
	must.Contains(t, stderr, `invalid emulator type "bogus"`)
}
...
func TestStartTypeFlagRefusesSwitchWhenDifferentEmulatorRunning(t *testing.T) {
	...
	must.Error(t, err)
	must.Contains(t, stdout, "LocalStack Azure Emulator is running on port 4566")
	must.Contains(t, stdout, "config was not changed")
	must.Contains(t, stdout, "docker stop localstack-external-azure")
