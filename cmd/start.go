...
Use --type (aws, snowflake, azure) to select the emulator non-interactively; it records the selection in config, switching the configured type in place when it differs. The type may also be given as a positional argument.
...
If a snapshot is configured for the AWS emulator (the snapshot field in [[containers]]), it is auto-loaded once the emulator starts. Use --snapshot REF to override it for one run, or --no-snapshot to skip it.`,
		...
		RunE: func(c *cobra.Command, args []string) error {
			...
			emulatorType, err := resolveEmulatorTypeFlag(c, args)
			if err != nil {
				return err
			}
