...
// split them across two destinations.
...
// completionSetup is the only copy of lstk's shell-completion setup docs: the
// per-shell subcommands and `lstk docs` both render it. %[1]s is the binary
// name. Command lines must stay indented — wrapText (cmd/help.go) reflows
// unindented prose and would break a command mid-word.
...
// documentCompletionCommands replaces Cobra's autogenerated completion help:
// it recommends `source <(… bash)`, a no-op on macOS bash 3.2 (DEVX-950), and
// gives PowerShell no persist path.
