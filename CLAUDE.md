...
Separately from how a tool is *run*, each proxy exec site declares *whose* failure a non-zero exit is by passing the result through `proc.MarkUserToolExit`: `lstk aws`, `terraform`, `cdk`, `sam`, and the `lstk az` passthrough (`azurecli.Exec` — **not** `azurecli.Run`, which is lstk's own orchestration). Extensions are **not** marked: they are lstk's own code shipped separately, so their exits are lstk's. `cmd.commandResult` turns the mark, the `proxyCommandAnnotation` on the five proxy commands, and lstk's own signal context (or `proc.WasInterrupted` on the interactive PTY path) into `parameters.proxied`, `result.proxy_exit_code` and `result.cancelled` on `lstk_command` events (DEVX-1004); the pipe derives whose failure it was from those. Contract: `openspec/changes/distinguish-proxied-command-errors/design.md`.
...
- User-facing failures are output too: surface them with `return output.Fail(sink, output.ErrorEvent{Title, Summary, Actions, Code}, err)` (use `Actions` for actionable next-steps, e.g. a command to run; always set a `Code` from `internal/output/error_code.go`). `Fail` emits the event and returns a `SilentError` carrying the code, which is how `result.error_code` reaches command telemetry — the two-step `sink.Emit(...)` then `return output.NewSilentError(err)` drops it. Domain code that returns an error for the caller to render uses `output.WithCode(err, code)` instead. `TestEveryNewErrorEventSetsACode` (`internal/output`) fails when a file gains an `ErrorEvent` literal without a `Code`; when you classify an existing one, lower its file's count in the baseline. The top-level handler in `cmd/root.go` checks `output.IsSilent(err)` and skips re-printing silent errors; any non-silent error returned up the stack falls through to an unstyled `Error: %v` on stderr — that is the fallback for when no sink is available (e.g. logger setup), not the preferred path for a failure a sink could have rendered.
