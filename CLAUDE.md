...
lstk supports Git-style extensions: when `lstk <name>` is not a built-in command or alias, lstk resolves and execs an external `lstk-<name>` executable, forwarding arguments verbatim and propagating the exit code. Built-ins always win. Resolution order is built-ins → bundled dir (the directory of the symlink-resolved lstk executable) → `PATH`; there is no manifest. Runtime context is conveyed via `LSTK_EXT_API_VERSION` and `LSTK_EXT_CONTEXT` (JSON: `configDir`, optional `authToken`, `nonInteractive`, `json`, optional `sessionId` — lstk's telemetry session id, omitted when telemetry is disabled, so an extension's own telemetry can join lstk's `ext:<name>` event — optional `machineId` — lstk's anonymized machine id (the prepared hash), omitted alongside `sessionId` when telemetry is disabled, so an extension reports the same machine without re-deriving it — optional `endpointUrl` — the resolved `--endpoint-url`/`LSTK_ENDPOINT_URL`/`AWS_ENDPOINT_URL` value, conveyed verbatim and unvalidated (dispatch never rejects or probes it, unlike the built-ins' `rejectEndpointURL`), omitted when no source is set — and an `emulators` array, which stays local-Docker discovery and is independent of `endpointUrl`) — see `extension.Context`/`Environ` in `internal/extension/context.go`; dispatch and help listing are in `cmd/extension.go`. LocalStack's own bundled extensions ship as **one multi-call binary**, `bundled-extensions`, next to `lstk`, plus an `lstk-extensions.toml` listing the commands it provides; lstk execs it with `argv[0]` = `lstk-<name>`. The invariants are on `extension.BundledBinaryName`, `LoadBundle` and `Extension.Argv0`; the release pipeline (helper scripts in `scripts/bundled-extensions/`, `make test-scripts`) is in [extensions-bundling.md](docs/extensions-bundling.md) and the author-facing contract in [extensions-authoring.md](docs/extensions-authoring.md). `lstk update` on the binary channel installs lstk, the bundle, the toml and the licence documents as one set (`internal/update/extract.go`).

# Licensing of the Packages

The lstk core is Apache-2.0; the bundled extensions are proprietary. Every package therefore ships the documents of both halves at the root next to `lstk`: `LICENSE` and `THIRD_PARTY_NOTICES.txt` (the core), and `bundled-extensions.THIRD_PARTY_NOTICES.txt` plus, once legal publishes it, `bundled-extensions.LICENSE.txt` (the bundle, staged from the bundle archive by `fetch-bundled-extensions.sh` under the `bundled-extensions.` prefix). The same files are listed in four places: `BUNDLE_DOCUMENTS` in the fetch script (bare archive names), `archives.files` in `.goreleaser.yaml` and `STAGED_BUNDLE_DOCUMENTS` in `add-bundled-to-npm.sh` (staged names), and `discoverMembers` in `internal/update/extract.go`. `check-bundled-packaging-sync.sh` fails when the first three disagree; `extract.go` deliberately runs ahead and already names `bundled-extensions.LICENSE.txt`, so an updater in the field installs the terms the day a bundle ships them. Never describe the whole package as open source or plain Apache-2.0: README/CONTRIBUTING carry legal's scoping sentence verbatim, the npm platform packages are relabelled with the compound SPDX expression held in `add-bundled-to-npm.sh` (`PLATFORM_LICENSE`, the one place it lives), and the cask config carries no `license` because casks cannot render one. `THIRD_PARTY_NOTICES.txt` is generated by `scripts/third-party-notices/generate.sh` (`make notices`); `make check-notices` gates CI and the release, failing on a stale file or a licence outside the script's allowlist. A new licence there is a question for legal, not an allowlist edit.
...
When lstk's stdout and stderr are both terminals, `lstk aws` runs the child via `proc.RunInPTY` (in `internal/proc/pty_unix.go`) instead: the spinner path wraps the child's output in an `io.Writer`, which makes os/exec hand the child a pipe, and the frozen Python aws CLI then block-buffers stdout (8 KB, ignoring `PYTHONUNBUFFERED`) — streaming commands like `aws logs tail --follow` showed nothing until exit (DEVX-1026). The PTY makes the child see a terminal (line-buffered, colored output; stdout/stderr merged) while the master side is copied through the spinner's `StopOnWriteWriter`. When lstk's stdin is also a terminal, the PTY additionally becomes the child's stdin and controlling terminal, with the user's terminal switched to raw mode and its keystrokes pumped into the PTY — without that, the aws CLI's pager read keyboard input from the PTY (less ≥577 reads the terminal attached to its stderr) and looked frozen (DEVX-1049); Ctrl-C then reaches the child as SIGINT via the PTY's line discipline rather than the shared process group (see `wireInteractiveStdin`). With stdin redirected, stdin passes through untouched and no new session is created, so piped data never crosses a line discipline and Ctrl-C still arrives via the process group. Falls back to plain `proc.Run` when no PTY can be allocated (Windows, see `pty_windows.go`), and stays on pipes whenever stdout is redirected so pipelines never receive colors/CRLF.
...
- Keep comments short: 1–3 lines is the norm, ~6 is a ceiling that needs a real reason (a subtle invariant, upstream behaviour that will surprise the reader, a rejected alternative someone will otherwise re-attempt). Long comments are harder to read and go stale — they are a maintenance burden, not extra rigor.
- State each reason once, in the fewest words that carry it. Don't restate the code, the declaration's own name, or what the adjacent test already asserts, and don't narrate why the obvious path is safe. Where the design took some working out, record the constraint that came out of it rather than the path that led there — the constraint is what a future reader needs in order not to break it.
- If a declaration seems to need several unrelated paragraphs, that's a signal to split the code — not to grow the comment.
