...
- Never let a login test open a real browser tab. `internal/auth.New` accepts `auth.WithBrowserOpener(open func(string) error)` — unit/TUI tests inject a recorder (see `internal/ui/run_login_test.go`'s `browserRecorder`) and assert on the captured URL. The integration suite's `TestMain` points `LSTK_BROWSER_CMD` at a no-op for every test (`installNoopBrowserLauncher`); a CLI-binary test that asserts on the launched URL uses `fakeBrowserOpener` (`test/integration/login_test.go`).
- Don't let an integration test fall into the device-login flow by accident through an inherited `LOCALSTACK_AUTH_TOKEN`. A test that never needs a licensed emulator passes a fake token plus a mock `env.APIEndpoint` (`With(env.AuthToken, "test-token")`); one that does calls `env.Require(t, env.AuthToken)`.
