...
- **Dev-only env-var hooks use the `lstk_dev` build tag** (`go build -tags lstk_dev`). Pattern: a file pair per package — `foo.go` (`//go:build !lstk_dev`) returning the hardcoded production value, `foo_dev.go` (`//go:build lstk_dev`) consulting an `LSTK_DEV_*` env var first (see `internal/update/urls.go` / `urls_dev.go`, which let a local fake release server stand in for GitHub when hand-testing `lstk update`). Reuse this one tag for new hooks — don't mint per-feature tags (each tag is another build variant CI must compile or it rots, and granularity already exists per env var). Production/goreleaser builds never set the tag, so released binaries can't be redirected via environment. The Unit Tests CI job builds `-tags lstk_dev` to keep the variant compiling.
