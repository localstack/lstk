...
func TestErrorCode_AllErrorCodesIsComplete(t *testing.T) {
	...
	if len(allErrorCodes) != 35 {
		t.Errorf("expected 35 documented error codes, got %d — update this test's expectation alongside error-codes/spec.md if a code was intentionally added or removed", len(allErrorCodes))
	}
}
