...
LocalStack's own extensions (for example `lstk doctor`) are proprietary and
ship **inside every lstk release** as these files placed next to the `lstk`
binary:
...
| `bundled-extensions.THIRD_PARTY_NOTICES.txt` | The licences of the open-source libraries built into the bundle, generated in the private repository. Renamed from the bundle's `THIRD_PARTY_NOTICES.txt` at staging time so it cannot be taken for lstk's own. |

The lstk core is Apache-2.0 and the bundle is not, so a package that ships both
carries the licence documents of both: lstk's `LICENSE` and
`THIRD_PARTY_NOTICES.txt` (generated by `scripts/third-party-notices/generate.sh`,
gated by `make check-notices` in CI) next to the bundle's documents above.
The proprietary terms for the bundle (`bundled-extensions.LICENSE.txt`) are
not yet published by the private repository; once they are, they flow through
the same staging as the notices (see [What the private repository must
publish](#what-the-private-repository-must-publish)).
...
(`extension.BundledDir`). Each install method lands the files there without
any layout work of its own; the licence documents ride along the same way.
...
2. **Fetch and verify.**
   ...
   archive and stages only these members: the binary as
   `bundled/<os>_<arch>/bundled-extensions[.exe]` (mode 0755), the
   descriptions file as `bundled/lstk-extensions.toml`, and the bundle's
   licence documents as `bundled/bundled-extensions.<name>` (each taken once;
   every archive must carry an identical copy, which is what lets the next
   step verify one file rather than six). Nothing else is staged, and nothing
   else reaches a released package. The list of documents is
   `BUNDLE_DOCUMENTS` at the top of the script; `.goreleaser.yaml`,
   `add-bundled-to-npm.sh` and `internal/update` name the same staged files, so
   adding one is a one-line change in each. It fails if any of lstk's six target platforms
...
4. **Package.** GoReleaser adds the staged files to each archive at the root,
   next to lstk's own `LICENSE` and `THIRD_PARTY_NOTICES.txt`. The cask
   inherits them. `scripts/bundled-extensions/add-bundled-to-npm.sh` copies
   the bundle and its documents into the platform packages, registers them in
   each package's `files`, and sets those packages' `license` to the compound
   expression declared at the top of the script (the wrapper package ships no
   bundle and keeps the publisher's `Apache-2.0`).
...
- one archive per lstk target platform, named
  ...
  Windows), `lstk-extensions.toml` and `THIRD_PARTY_NOTICES.txt`. Anything
  else in the archive is ignored; only those files are staged and shipped;
...
- the same `THIRD_PARTY_NOTICES.txt` in every archive: the licences of the
  libraries built into the binary. lstk ships it as
  `bundled-extensions.THIRD_PARTY_NOTICES.txt`. The bundle's proprietary
  terms will travel the same way once legal has drafted them, as
  `LICENSE.txt` in the archive and `bundled-extensions.LICENSE.txt` in lstk's
  packages; that file is expected to come from this repository so the terms
  are versioned with the binary they cover;
...
every platform, a toml describing one placeholder `doctor` command and a
labelled placeholder for each of the bundle's licence documents, and prints a
banner: **artifacts built from a stub bundle must never be released.**
...
`bundled/` while the release job has no fetch step, or the reverse, and if the
bundle's licence-document list differs between the fetch script,
`.goreleaser.yaml`, `add-bundled-to-npm.sh` and `internal/update/extract.go`.
`goreleaser check` cannot catch this itself because it only validates config
syntax and never looks at the filesystem. All halves must land in the same
PR.
...
npx --yes goreleaser-npm-publisher@1.5.0 build --project . --prefix @localstack \
  ...
  --files README.md LICENSE THIRD_PARTY_NOTICES.txt
...
the binary, the bundle, the toml and the licence documents are always replaced
together, including renames and removals.

**Binary channel** (`lstk update` downloading an archive itself): the updater
installs the set as one unit: `lstk`, `bundled-extensions`,
`lstk-extensions.toml` and the licence documents at the archive root
(`LICENSE`, `THIRD_PARTY_NOTICES.txt`, `bundled-extensions.LICENSE.txt`,
`bundled-extensions.THIRD_PARTY_NOTICES.txt`), lstk last so a failure part-way
leaves a working lstk. An archive that lacks a document (one published before
it existed) still updates. Updating never deletes standalone `lstk-<name>`
files a user placed next to the binary.
