# terraform-proxy Specification

## Purpose

Provide an `lstk terraform` command (alias `lstk tf`) that proxies the Terraform CLI against a running LocalStack AWS emulator, automatically generating provider override files so Terraform targets LocalStack endpoints.
