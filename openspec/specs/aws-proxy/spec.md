# aws-proxy Specification

## Purpose

Define the credentials `lstk aws` supplies to the wrapped AWS CLI, the `--account` flag that selects which LocalStack account a call addresses, and how an explicit account selection interacts with the `localstack` AWS profile.

This capability is deliberately scoped to credentials and account selection. The rest of the `lstk aws` contract — endpoint resolution, help short-circuiting, shell completion, and PTY handling — is existing behavior that this capability does not yet describe, and will be added over time.
