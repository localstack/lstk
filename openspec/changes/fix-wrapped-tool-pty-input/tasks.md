...
> Implemented following the proposed decision in `design.md` (Option B: full virtualization). If the team decides for Option A instead, sections 1 and 2 are revised; the change is contained in `internal/proc`.
...
- [x] 1.2 Cancelreader-based pump teardown (cancel, join, close, surface restore errors)
...
- [x] 2.2 faketool `Pager` mode (raw read from fd 2)
