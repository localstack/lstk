...
```
Codes: `RUNTIME_UNAVAILABLE`, `EMULATOR_NOT_RUNNING` (found during implementation: `stop` fails fast on the first configured emulator not running, matching existing plain-text behavior, so this code was missing from the original catalog entry).
