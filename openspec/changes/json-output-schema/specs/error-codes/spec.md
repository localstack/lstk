...
|---|---|---|---|
| `RUNTIME_UNAVAILABLE` | The container runtime (`internal/runtime.Runtime` — Docker today; Podman, Rancher Desktop, Finch, or Kubernetes are architecturally anticipated but not yet implemented) is unreachable or unhealthy | Yes | `RUNTIME` |
...
- **WHEN** the same `error.code` is emitted by two different commands
- **THEN** both error objects report the same `category` value for that code

#### Scenario: An unclassified failure's category matches its fallback code
- **WHEN** a JSON-capable command hits a failure that has not been mapped to a specific code (so `error.code` is `"INTERNAL_ERROR"`)
- **THEN** `error.category` is `"INTERNAL"`
