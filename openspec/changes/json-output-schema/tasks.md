...
- [x] 5.1 `start` (`cmd/start.go` and the root command's bare invocation — bare `lstk` runs through the identical `startEmulator` path, so it carries the same annotation and JSON-aware sink; `isExtensionDispatch` already exempts the args-present path from this regardless): add annotation, a flat `{"emulator", "container", "endpoint", "version", "alreadyRunning", "persistence"}` data shape (not an `emulators: [...]` list — only one `[[containers]]` block can be enabled at a time, so `start` always acts on exactly one emulator), classify `RUNTIME_UNAVAILABLE`/`AUTH_REQUIRED`/`LICENSE_INVALID`/`LICENSE_NOT_COVERED` (new code, added alongside `PORT_CONFLICT` — see `error-codes` spec)/`IMAGE_PULL_FAILED`/`EMULATOR_START_FAILED`/`EMULATOR_WRONG_TYPE`/`PORT_CONFLICT`/`CONFIG_INVALID` at their existing call sites. Domain change: `container.Start` now returns a `StartResult` struct instead of a bare version string, since the JSON payload needs real connection info (endpoint, container name, already-running state) that the old return value couldn't carry — the empty-string "already running" sentinel becomes an explicit `AlreadyRunning bool` field, threaded through to the two other callers (`cmd/root.go`'s bare-root path, `internal/ui/run.go`'s TUI path) that gated snapshot auto-load / label-cache reuse on it. `start --json` stays silent until the final line, same as `stop`/`reset`/`update` — no separate stderr progress channel.
...
- [x] 5.4a Integration tests for `start`: fresh start, already-running no-op, `PORT_CONFLICT`, `LICENSE_INVALID` — see `test/integration/start_json_test.go`. `start` turned out not to need the `status`-style "report all configured emulators" scenario: only one `[[containers]]` block can be enabled at a time, so there is only ever one emulator to report.
- [ ] 5.4b Integration tests for `restart` and `volume clear`, the remaining commands in this wave.
