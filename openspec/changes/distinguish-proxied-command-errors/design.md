...
**It is emitted raw.** An invocation that returned `nil` with a done context records `cancelled: true, exit_code: 0`. The pipe's `error_source` puts `exit_code = 0 → none` first, so a success is never counted as a cancellation there; the raw field keeps the observation ("a signal arrived") intact for anyone who wants it. `cancelled` must not be read as "this was a failure" on its own.
...
- **The repo already has the why axis, unused here.** `internal/output/error_code.go` ships 34 `ErrorCode` values and an 8-value `ErrorCategory` whose `CategoryRuntime` is documented as "outside lstk's control". Telemetry cannot read it: the code rides the `ErrorEvent` the sink consumes, not the error value `instrumentCommands` receives, and only 20 of 68 emit sites set one. Wiring it in is the change that would make a real reliability metric possible — a separate ticket.
