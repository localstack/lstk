...
- **Add `parameters.proxied`** (bool, always emitted): the invocation asked lstk to run a wrapped tool, regardless of whether that tool started or succeeded. True for `aws`, `az`, `cdk`, `sam`, `terraform`, and extensions; false for lstk's own commands, including `az start-interception`/`stop-interception`.
...
- **Determine the origin at the exec site, never from the error's type.** `proc.MarkUserToolExit` marks the six places that run a tool the user asked for. lstk shells out for its own purposes too (`brew upgrade`, `az cloud list`, `aws s3api create-bucket`), and those exits read `exit status N` exactly like the user's.
